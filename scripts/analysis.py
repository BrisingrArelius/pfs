...
# Size suffixes (102mb, 1gb, 10gb) and storage tags (hdd, ssd) are preserved.
# =============================================================================
...
def abbreviate(label):
    """
    Shorten a profile label for use in plots.
    Preserves size suffixes (102mb, 1gb, 10gb) and storage tags (hdd, ssd).
    Example: large_contiguous_read_heavy_freq_1gb → L_cont_rh_fr_1gb
    """
    s = label
    for full, short in ABBREV_MAP:
...
def parse_args():
    ...
    input_group.add_argument("--input", help="Path to single global.csv file")
    ...
    parser.add_argument("--top-n",      type=int, default=TOP_N_COUNTERS,
                        help=f"Number of top discriminative counters (default: {TOP_N_COUNTERS})")
...
def compute_derived_metrics(df):
    ...
    if all(c in df.columns for c in ['POSIX_BYTES_READ', 'POSIX_BYTES_WRITTEN',
                                      'POSIX_READ_DURATION', 'POSIX_WRITE_DURATION']):
    ...
    if all(c in df.columns for c in ['POSIX_SEEKS', 'POSIX_READS', 'POSIX_WRITES']):
        total_ops = df['POSIX_READS'] + df['POSIX_WRITES']
        df['POSIX_SEEK_RATE'] = df['POSIX_SEEKS'] / total_ops.replace(0, np.nan)

    derived_count = sum(
        1 for c in df.columns
        if any(x in c for x in ['DURATION', 'BW_', 'LATENCY_', 'DENSITY', 'MEAN_', 'RATIO', 'RATE'])
    )
...
def identify_stable_counters(stats, counter_cols, cv_threshold):
    ...
    stable = [
        ...
        if (cv_vals := stats[(col, 'cv')].dropna()).size > 0
        and cv_vals.max() < cv_threshold
    ]
...
def identify_discriminative_counters(stats, counter_cols, top_n):
    ...
    scores = {
        col: (std / mean if (mean := stats[(col, 'mean')].mean()) != 0 else 0)
        for col in counter_cols
        for std in [stats[(col, 'mean')].std()]
    }
...
def plot_heatmap(stats, counter_cols, output_path, title, cmap='YlOrRd'):
    ...
    # Abbreviate y-axis (profile) labels
    abbreviated_index = [abbreviate(str(v)) for v in normalized.index]
    ...
    ax = sns.heatmap(
        ...
        xticklabels=True,
        yticklabels=abbreviated_index,
        linewidths=0.5
    )
    plt.title(title, fontsize=16, pad=20)
    plt.xlabel('Counters', fontsize=12)
    plt.ylabel('Workload Profiles', fontsize=12)
    plt.xticks(rotation=90, fontsize=max(7, min(11, 120 // max(len(counter_cols), 1))))
...
def plot_bar_charts(stats, top_counters, output_dir):
    ...
    Uses abbreviated profile labels on x-axis.
    """
    print("\nGenerating bar charts for discriminative counters...")

    n_counters = len(top_counters)
    n_cols = 2                                      # 2 columns → more vertical room
    n_rows = (n_counters + n_cols - 1) // n_cols

    # Scale figure height so labels have room
    fig_height = max(n_rows * 5, 12)
    ...
    for i, counter in enumerate(top_counters):
        ...
        abbrev_labels = [abbreviate(str(p)) for p in means.index]

        x_pos = np.arange(len(means))
        ax.bar(x_pos, means, yerr=stds, capsize=4, alpha=0.75, color='steelblue',
               error_kw={'elinewidth': 1.2})
        ax.set_xticks(x_pos)
        ax.set_xticklabels(abbrev_labels, rotation=55, ha='right', fontsize=8)
    ...
    plt.suptitle('Top Discriminative Counters Across Workloads', fontsize=16, y=1.01)
    plt.tight_layout()
...
def plot_pca(stats, counter_cols, output_dir):
    ...
    Uses abbreviated labels in the legend (no inline annotations to avoid overlap).
    """
    print("\nGenerating PCA scatter plot...")

    if len(stats) < 2:
        print(f"  Skipping PCA — need at least 2 data points (have {len(stats)})")
        return
    ...
    for i, profile in enumerate(profiles):
        abbrev = abbreviate(profile)
        ax.scatter(
            reduced[i, 0], reduced[i, 1],
            color=cmap(i), s=220, alpha=0.85,
    ...
    ax.set_title('PCA: Workload Clustering Based on I/O Counters', fontsize=16, pad=16)
    ax.grid(alpha=0.3)

    # Legend outside plot — two columns for readability
    ax.legend(
        bbox_to_anchor=(1.02, 1), loc='upper left',
        fontsize=9, ncol=2,
        title='Profile (abbreviated)', title_fontsize=9
    )
...
# =============================================================================
# HDD vs SSD COMPARISON HELPERS
# =============================================================================

def _plot_side_by_side_bars(ax, stats_hdd, stats_ssd, metric, title, ylabel, color, use_log=False):
    """Side-by-side HDD/SSD bar chart on a given axis. Returns True on success."""
    available = [col for col, _ in stats_hdd.columns]
    ...
    if not common_profiles:
        ax.text(0.5, 0.5, 'No common profiles', ha='center', va='center', fontsize=11)
        ax.set_title(title)
        return False
    ...
    ax.bar(x - width/2, hdd_data, width, label='HDD', alpha=0.82, color='orange')
    ax.bar(x + width/2, ssd_data, width, label='SSD', alpha=0.82, color=color)

    abbrev_labels = [abbreviate(p) for p in common_profiles]
    ax.set_xlabel('Workload Profile', fontsize=10)
    ax.set_ylabel(ylabel, fontsize=10)
    ax.set_title(title, fontsize=12, fontweight='bold')
    ax.set_xticks(x)
    ax.set_xticklabels(abbrev_labels, rotation=55, ha='right', fontsize=7)
    ax.legend(fontsize=9)
    ax.grid(axis='y', alpha=0.3)

    if use_log:
        all_vals = [v for v in hdd_data + ssd_data if v > 0]
...
def plot_interleaved_heatmap(stats_hdd, stats_ssd, counter_cols, output_path):
    ...
    plt.xticks(rotation=90, fontsize=max(7, min(11, 150 // max(len(counter_cols), 1))))
...
def plot_bandwidth_comparison(stats_hdd, stats_ssd, output_dir):
    ...
    for idx, (metric, title, color) in enumerate(bw_metrics):
        _plot_side_by_side_bars(axes[idx], stats_hdd, stats_ssd, metric,
                                title, 'Bandwidth (MB/s)', color)
...
def plot_latency_comparison(stats_hdd, stats_ssd, output_dir):
    ...
    for idx, (metric, title, color) in enumerate(latency_metrics):
        _plot_side_by_side_bars(axes[idx], stats_hdd, stats_ssd, metric,
                                title, 'Latency (ms)', color, use_log=True)

    plt.suptitle('Operation Latency: HDD vs SSD Comparison', fontsize=16, y=1.01)
    plt.tight_layout()
...
def plot_performance_gains(stats_hdd, stats_ssd, output_dir):
    """
    Split into two separate PNGs:
      performance_gains_bandwidth.png  — bandwidth speedup (SSD / HDD)
      performance_gains_latency.png    — latency reduction (HDD / SSD)
    Each plot is wide enough that abbreviated x-axis labels are readable.
    """
    print("\nGenerating performance gain analysis (split into two files)...")
    ...
    available_hdd = [col for col, _ in stats_hdd.columns]
    ...
    for metric in bw_metrics:
        if metric not in available_hdd:
            continue
        metric_short = metric.replace('POSIX_', '').replace('_BW_MBps', '')
        hdd_vals = stats_hdd[(metric, 'mean')].dropna()
        ssd_vals = stats_ssd[(metric, 'mean')].dropna()
        ...
        for profile in common:
            if hdd_vals.loc[profile] > 0:
                ...
                # Two-line label: abbreviated profile + metric tag
                speedup_labels.append(f"{abbreviate(profile)}\n[{metric_short}]")
                speedup_colors.append('green' if speedup >= 1 else 'red')

    if speedup_data:
        n = len(speedup_data)
        # Give ~0.55 inch per bar so labels don't pile up
        fig_w = max(18, n * 0.55)
        fig, ax = plt.subplots(figsize=(fig_w, 8))

        x = np.arange(n)
        ax.bar(x, speedup_data, color=speedup_colors, alpha=0.78, edgecolor='black', width=0.65)
        ax.axhline(y=1, color='black', linestyle='--', linewidth=1.8, label='Baseline (HDD=1×)')
        ax.set_xticks(x)
        ax.set_xticklabels(speedup_labels, rotation=55, ha='right', fontsize=7)
        ax.set_xlabel('Profile & Metric', fontsize=12)
        ax.set_ylabel('Speedup Factor (SSD / HDD)', fontsize=12)
        ax.set_title('Bandwidth Speedup: SSD vs HDD', fontsize=15, fontweight='bold', pad=14)
        ax.legend(fontsize=10)
        ax.grid(axis='y', alpha=0.3)
        plt.tight_layout()

        out_bw = os.path.join(output_dir, 'performance_gains_bandwidth.png')
        plt.savefig(out_bw, dpi=300, bbox_inches='tight')
    ...
    for metric in lat_metrics:
        if metric not in available_hdd:
            continue
        metric_short = metric.replace('POSIX_', '').replace('_LATENCY_ms', '')
        hdd_vals = stats_hdd[(metric, 'mean')].dropna()
        ssd_vals = stats_ssd[(metric, 'mean')].dropna()
        common   = sorted(set(hdd_vals.index) & set(ssd_vals.index))
        ...
        for profile in common:
            if ssd_vals.loc[profile] > 0:
                ...
                reduction_labels.append(f"{abbreviate(profile)}\n[{metric_short}]")
                reduction_colors.append('green' if reduction >= 1 else 'red')

    if reduction_data:
        n = len(reduction_data)
        fig_w = max(18, n * 0.55)
        fig, ax = plt.subplots(figsize=(fig_w, 8))

        x = np.arange(n)
        ax.bar(x, reduction_data, color=reduction_colors, alpha=0.78, edgecolor='black', width=0.65)
        ax.axhline(y=1, color='black', linestyle='--', linewidth=1.8, label='Baseline (HDD=1×)')
        ax.set_xticks(x)
        ax.set_xticklabels(reduction_labels, rotation=55, ha='right', fontsize=7)
        ax.set_xlabel('Profile & Metric', fontsize=12)
        ax.set_ylabel('Latency Reduction Factor (HDD / SSD)', fontsize=12)
        ax.set_title('Latency Improvement: SSD vs HDD', fontsize=15, fontweight='bold', pad=14)
        ax.legend(fontsize=10)
        ax.grid(axis='y', alpha=0.3)
        plt.tight_layout()

        out_lat = os.path.join(output_dir, 'performance_gains_latency.png')
        plt.savefig(out_lat, dpi=300, bbox_inches='tight')
